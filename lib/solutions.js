...
// Helper function to search for solutions
export async function searchSolutions(query, limit = 20) {
  if (!query) return [];

  const searchTerm = query.trim().toLowerCase();

  // Search in multiple columns using ilike (case-insensitive matching)
  const { data, error } = await supabase
    .from("solutions")
    .select("*")
    .or(
      `name.ilike.%${searchTerm}%, description.ilike.%${searchTerm}%, overview.ilike.%${searchTerm}%, category.ilike.%${searchTerm}%`
    )
